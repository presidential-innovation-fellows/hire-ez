...
Class NotificationParser {
  ...
  public static function parse($notification) {
    ...
    if ($notification->notification_type == "Dismissal") {
      ...
      $bid = $notification->payload["bid"];
      $return_array["subject"] = $bid["vendor"]["name"]." has submitted a bid for ".$bid["project"]["title"].".";
      $return_array["line1"] = $bid["vendor"]["name"]." has <a href='".route('bid', array($bid["project"]["id"], $bid["id"])).
                "'>submitted a bid</a> for ".$bid["project"]["title"].".";
      $return_array["line2"] = Helper::truncate($bid["body"], 18);
      $return_array["link"] = route('bid', array($bid["project"]["id"], $bid["id"]));
